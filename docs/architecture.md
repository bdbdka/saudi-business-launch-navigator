...
Browser
  -> Next.js interface
  -> FastAPI application
       -> deterministic questionnaire and checklist services
       -> PostgreSQL catalog and versioned conditions
       -> optional OpenAI interpretation and explanation
```
...
Every demo response is visibly identified as sample data. Example source links
use reserved `.invalid` domains and must not be interpreted as official
evidence.
